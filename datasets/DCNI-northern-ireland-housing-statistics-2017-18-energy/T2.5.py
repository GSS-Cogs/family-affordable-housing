...
scheme = cell.fill(DOWN).is_not_blank().is_not_whitespace()
observations = scheme.fill(RIGHT).is_not_blank().is_not_whitespace()
measure = cell.shift(0,1).fill(RIGHT).is_not_blank().is_not_whitespace()
Dimensions = [
            HDim(Year,'Year',CLOSEST,LEFT),
            HDim(scheme,'NI Household Energy',DIRECTLY,LEFT ),
            HDim(measure,'Unit',DIRECTLY,ABOVE),  
            HDim(measure,'Measure Type',DIRECTLY,ABOVE),
            HDimConst('NI Household Description','Affordable Warmth Scheme')
