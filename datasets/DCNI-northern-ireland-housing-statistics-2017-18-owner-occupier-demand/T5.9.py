# ---
# jupyter:
#   jupytext:
#     formats: ipynb,py:light
#     text_representation:
#       extension: .py
#       format_name: light
#       format_version: '1.5'
#       jupytext_version: 1.3.3
#   kernelspec:
#     display_name: Python 3
#     language: python
#     name: python3
# ---

from gssutils import *

# +
cell = tab.filter(contains_string('Year'))
cell.assert_one()
Year = cell.fill(DOWN).is_not_blank().is_not_whitespace() 
quarter = cell.fill(RIGHT).is_not_blank().is_not_whitespace() 
observations = quarter.fill(DOWN).is_not_blank().is_not_whitespace() - tab.excel_ref('F17')
Dimensions = [
            HDim(Year,'Year', DIRECTLY,LEFT),
            HDim(quarter, 'quarter',DIRECTLY,ABOVE )
            HDimConst('NI Housing Specification','Mortgage'),
            HDimConst('Unit','applications'),  
            HDimConst('Measure Type','count'),
            HDimConst('NI Owner Occupied Housing Description','Mortgage Cases Received')
    
]  
c1 = ConversionSegment(observations, Dimensions, processTIMEUNIT=True)
new_table = c1.topandas()
import numpy as np
new_table.rename(columns={'OBS': 'Value','DATAMARKER': 'NI Marker'}, inplace=True)
new_table['quarter'] = new_table['quarter'].map(
    lambda x: {
        'Apr - Jun' : 'Q1', 
        'Jul - Sep' : 'Q2', 
        'Oct - Dec' : 'Q3', 
        'Jan - Mar' : 'Q4',
        'Yearly Total' : ''
        }.get(x, x))
table['Period'] =  'year/' + table['Year'].astype(str).str[:4] + '-' + new_table['quarter']
table['Period'] = table['Period'].str.rstrip('-')
